...
class TestFormatValidationReport:
    ...
    def test_format_report_escapes_markup(self):
        ...
        report = ValidationReport()
        report.add_error("test[bold]field[/bold]", "Error with [red]markup[/red]", "Fix [green]this[/green]")
...
""")
        ...
        try:
            ...
            result = compare_configurations(config1, config2, file1_name="file1", file2_name="file2")
...
""")
        ...
        try:
            ...
            result = compare_configurations(config1, config2, ignore_fields=["metadata.created_date"])
            assert not result["has_differences"]
        finally:
            temp_path1.unlink()
            temp_path2.unlink()
...
class TestSuggestConfigLocation:
    ...
    def test_suggest_existing_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('metareason.cli.utils.discover_config_directories', return_value=[temp_path]):
