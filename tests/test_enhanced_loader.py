...
""")
        ...
        try:
            with pytest.raises(EnvironmentSubstitutionError):
                load_yaml_config(temp_path, enable_env_substitution=True, env_strict=True)
        finally:
            temp_path.unlink()
...
""")
        ...
        try:
            ...
            with pytest.raises(Exception):  # Will fail on validation due to broken include
...
""")
            ...
            config = load_yaml_config(
                "test.yaml",
                search_paths=[str(search1), str(search2)]
            )
