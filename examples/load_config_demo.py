...
def demo_basic_loading():
    ...
    print(f"🔍 Oracles: {sum([1 for o in [config.oracles.accuracy, config.oracles.explainability, config.oracles.confidence_calibration] if o])}")
    print()
...
def demo_caching():
    ...
    print(f"📊 Cache stats: {stats['active_entries']} active, TTL: {stats['ttl_seconds']}s")
