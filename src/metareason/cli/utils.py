...
def format_validation_report(report: ValidationReport, file_path: Optional[str] = None) -> str:
    ...
    if report.errors:
        ...
        for i, error in enumerate(report.errors, 1):
            lines.append(f"  {i}. [red]{error['field']}[/red]: {escape(error['message'])}")
...
def create_config_summary_table(config: EvaluationConfig) -> Table:
    ...
    categorical_count = sum(1 for axis in config.axes.values() if hasattr(axis, 'values'))
