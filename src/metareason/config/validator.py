...
def _add_warnings_and_suggestions(
    ...
    if unused_axes:
        ...
        if strict:
            report.add_error("axes", msg, "Remove unused axes or use them in prompt_template")
        else:
    ...
    if not config.metadata:
        ...
        if not config.metadata.review_cycle:
            report.add_suggestion("Add review_cycle to metadata for maintenance planning")
