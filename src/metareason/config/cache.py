...
class ConfigCache:
        ...
        with self._lock:
            ...
            if self._enable_hot_reload and path.exists():
                try:
                    ...
                    del self._cache[cache_key]
                    return None
            elif self._enable_hot_reload and not path.exists() and cache_entry["mtime"] != 0.0:
                # File was deleted
                del self._cache[cache_key]
                return None
    ...
    def set(self, file_path: Union[str, Path], config: EvaluationConfig) -> None:
        ...
        try:
            mtime = path.stat().st_mtime if self._enable_hot_reload and path.exists() else 0.0
        except (OSError, IOError):
            mtime = 0.0
