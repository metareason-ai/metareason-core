...
def load_yaml_config(
    ...
    try:
        ...
        if (
            ...
            or "access is denied" in str(e).lower()
        ):
            raise PermissionError(
                f"Permission denied reading configuration file: {path}. "
                f"Suggestion: Check file permissions with 'ls -la {path}'"
...
def validate_yaml_string(yaml_content: str) -> EvaluationConfig:
    ...
    if "oracles" in data:
        ...
        if "accuracy" in oracle_data:
            oracle_config_data["accuracy"] = EmbeddingSimilarityOracle(
                **oracle_data["accuracy"]
            )

        if "explainability" in oracle_data:
            oracle_config_data["explainability"] = LLMJudgeOracle(
                **oracle_data["explainability"]
            )

        if "confidence_calibration" in oracle_data:
            oracle_config_data["confidence_calibration"] = StatisticalCalibrationOracle(
                **oracle_data["confidence_calibration"]
            )
...
def _resolve_config_path(
    file_path: Union[str, Path], search_paths: Optional[List[Union[str, Path]]] = None
) -> Path:
    ...
    default_search_paths = [
        ...
        (
            Path("/etc/metareason")
            if os.name != "nt"
            else Path("C:/ProgramData/metareason")
        ),
    ]
