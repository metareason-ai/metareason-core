...
class PriorConfig(BaseModel):
    ...
    alpha: float = Field(
        default=1.0, gt=0, description="Alpha parameter for Beta prior"
    )
    beta: float = Field(default=1.0, gt=0, description="Beta parameter for Beta prior")
...
class InferenceConfig(BaseModel):
    ...
    method: Literal["mcmc", "variational", "exact"] = Field(
        default="mcmc", description="Inference method to use"
    )
    samples: int = Field(
        default=4000, ge=1000, le=100000, description="Number of samples for MCMC"
    )
    chains: int = Field(default=4, ge=1, le=16, description="Number of MCMC chains")
    target_accept: float = Field(
        default=0.8, ge=0.6, le=0.99, description="Target acceptance rate for MCMC"
    )
...
class OutputConfig(BaseModel):
    ...
    hdi_method: Literal["shortest", "central"] = Field(
        default="shortest", description="Method for computing highest density interval"
    )
...
class StatisticalConfig(BaseModel):
    ...
    model: Literal["beta_binomial", "binomial", "gaussian"] = Field(
        default="beta_binomial", description="Statistical model to use"
    )
    prior: PriorConfig = Field(
        default_factory=PriorConfig, description="Prior distribution configuration"
    )
    inference: InferenceConfig = Field(
        default_factory=InferenceConfig, description="Inference method configuration"
    )
